...
function initCarousel() {
  ...
  window.carouselImages.forEach((_, idx) => {
    ...
    dot.className = idx === 0 ? 'w-3 h-3 bg-white rounded-full' : 'w-3 h-3 bg-gray-400 rounded-full hover:bg-gray-300';
...
function updateCarousel() {
  ...
  dots.forEach((dot, idx) => {
    if (idx === currentSlide) {
      ...
      dot.className = 'w-3 h-3 bg-gray-400 rounded-full hover:bg-gray-300';
    }
  });
}
  ...
  filterBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      filterBtns.forEach(b => {
        b.classList.remove('bg-purple-700', 'text-white', 'font-medium');
        b.classList.add('border-2', 'border-purple-700', 'text-purple-700');
      });
      
      e.target.classList.add('bg-purple-700', 'text-white', 'font-medium');
      e.target.classList.remove('border-2', 'border-purple-700', 'text-purple-700');
...
function renderOrgCards(searchQuery = '', filter = 'all') {
  ...
  let filtered = window.orgsData.filter(org => {
    const matchSearch = searchQuery === '' || org.name.toLowerCase().includes(searchQuery);
  ...
  if (filtered.length === 0) {
    grid.innerHTML = '<div class="col-span-full text-center text-gray-500 py-8">Organisasi tidak ditemukan</div>';
    return;
  }

  filtered.forEach(org => {
    const card = document.createElement('div');
    card.className = 'bg-white rounded-lg shadow-md hover:shadow-lg transition overflow-hidden org-card';
    card.innerHTML = `
